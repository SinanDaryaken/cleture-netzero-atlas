...
from sources.glec.parser import GlecDocument, GlecParser


class GlecAdapter(BaseAtlasSourceAdapter):
    VERSION = "3.2"
    ...
    def __init__(
        ...
        *,
        ...
        parser: GlecParser | None = None,
        normalizer: GlecNormalizer | None = None,
    ) -> None:
        super().__init__(repository=repository, storage=storage)
        self.fetcher = fetcher or HttpAssetFetcher()
        self.parser = parser or GlecParser()
        self.normalizer = normalizer or GlecNormalizer(Path(__file__).with_name("mappings.yaml"))
        self._asset: FetchedAsset | None = None
        self._document: GlecDocument | None = None

    async def check(self, context: PipelineContext) -> ChangeCheckResult:
        download_url = context.source.location.download_url
        if download_url is None:
            raise ValueError("GLEC manifest requires an official PDF download URL")
        context.source_dataset_version = self.VERSION
        context.source_published_at = datetime(2025, 10, 21, tzinfo=UTC)
    ...
    async def parse(self, context: PipelineContext) -> tuple[ParsedRecord, ...]:
        if context.fetched_asset is None or context.fetched_asset.local_path is None:
            raise ValueError("GLEC parser requires a local PDF path")
    ...
    async def normalize(self, context: PipelineContext) -> tuple[CanonicalFactor, ...]:
        if context.raw_asset is None or self._document is None:
            raise ValueError("GLEC parsed document is unavailable")
        factors, observations, metrics = self.normalizer.normalize(
            self._document.fuels,
            self._document.intensities,
            self._document.refrigerants,
    ...
    def _version_key(self, context: PipelineContext) -> str:
        if context.raw_asset is None:
            raise ValueError("GLEC raw metadata is unavailable")
