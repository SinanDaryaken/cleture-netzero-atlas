...
from sources.plastics_recyclers_europe.parser import (
    PlasticsRecyclersEuropeDocument,
    PlasticsRecyclersEuropeParser,
)


class PlasticsRecyclersEuropeAdapter(BaseAtlasSourceAdapter):
    VERSION = "2015-05-29"
    ...
    def __init__(
        ...
        *,
        ...
        parser: PlasticsRecyclersEuropeParser | None = None,
        normalizer: PlasticsRecyclersEuropeNormalizer | None = None,
    ) -> None:
        super().__init__(repository=repository, storage=storage)
        self.fetcher = fetcher or HttpAssetFetcher()
        self.parser = parser or PlasticsRecyclersEuropeParser()
        self.normalizer = normalizer or PlasticsRecyclersEuropeNormalizer()
        self._asset: FetchedAsset | None = None
        self._document: PlasticsRecyclersEuropeDocument | None = None

    async def check(self, context: PipelineContext) -> ChangeCheckResult:
        download_url = context.source.location.download_url
        if download_url is None:
            raise ValueError("PRE manifest requires an official PDF download URL")
        context.source_dataset_version = self.VERSION
        context.source_published_at = datetime(2015, 5, 29, tzinfo=UTC)
    ...
    async def parse(self, context: PipelineContext) -> tuple[ParsedRecord, ...]:
        if context.fetched_asset is None or context.fetched_asset.local_path is None:
            raise ValueError("PRE parser requires a local PDF path")
    ...
    async def normalize(self, context: PipelineContext) -> tuple[CanonicalFactor, ...]:
        if context.raw_asset is None or self._document is None:
            raise ValueError("PRE parsed document is unavailable")
        factors, observations, metrics = self.normalizer.normalize(
            self._document.resin_results,
            self._document.parameters,
            raw=context.raw_asset,
            dataset_version=self._document.version,
            parser_version=context.effective_parser_version,
        )
    ...
    def _version_key(self, context: PipelineContext) -> str:
        if context.raw_asset is None:
            raise ValueError("PRE raw metadata is unavailable")
        return f"{self.VERSION}-{context.raw_asset.sha256[:12]}"
