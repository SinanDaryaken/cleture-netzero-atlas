...
from sources.cbam.parser import CbamDocument, CbamParser


class CbamAdapter(BaseAtlasSourceAdapter):
    member_filenames: ClassVar[dict[str, str]] = {
        "default_values": "cbam-default-values-definitive-2026-v2.xlsx",
        "benchmarks": "cbam-benchmarks-definitive-2026-v1.xlsx",
    }
    ...
    def __init__(
        ...
        *,
        ...
        parser: CbamParser | None = None,
        normalizer: CbamNormalizer | None = None,
    ) -> None:
        super().__init__(repository=repository, storage=storage)
        self.fetcher = fetcher or HttpAssetFetcher()
        self.parser = parser or CbamParser()
        self.normalizer = normalizer or CbamNormalizer(
            Path(__file__).with_name("mappings.yaml")
        )
        self._bundle: FetchedAsset | None = None
        self._document: CbamDocument | None = None
    ...
    async def check(self, context: PipelineContext) -> ChangeCheckResult:
        ...
        context.source_published_at = datetime(2026, 8, 10, tzinfo=UTC)
        context.metrics["bundle_members"] = len(self.member_filenames)
        ...
        result = ChangeCheckResult(
            ...
            reason=(
                f"CBAM {dataset_version} official workbook bundle changed"
                if changed
                else f"CBAM {dataset_version} official workbook bundle unchanged"
            ),
        )
    ...
    async def fetch(self, context: PipelineContext) -> FetchedAsset:
        ...
        if self._bundle is None:
            raise ValueError("check must build the CBAM acquisition bundle")
        return self._bundle
    ...
    async def parse(self, context: PipelineContext) -> tuple[ParsedRecord, ...]:
        if context.fetched_asset is None or context.fetched_asset.local_path is None:
            raise ValueError("CBAM parser requires the local acquisition bundle")
    ...
    async def normalize(self, context: PipelineContext) -> tuple[CanonicalFactor, ...]:
        if context.raw_asset is None or self._document is None:
            raise ValueError("CBAM parsed workbook bundle is unavailable")
        dataset_version = context.source.history.snapshot_parameters["dataset_version"]
        factors, observations, metrics = self.normalizer.normalize(
            self._document.default_values,
            self._document.benchmarks,
    ...
    async def _build_bundle(
        ...
        for role in ("default_values", "benchmarks"):
            ...
            try:
                if fetched.local_path is None:
                    raise ValueError("CBAM member download requires a local path")
        ...
        temporary = tempfile.NamedTemporaryFile(prefix="atlas-cbam-", suffix=".zip", delete=False)
        temporary.close()
        ...
        return (
            FetchedAsset(
                filename=f"cbam-{dataset_version}-{revision[:12]}.zip",
                local_path=path,
    ...
    def _version_key(self, context: PipelineContext) -> str:
        if context.raw_asset is None or self._revision is None:
            raise ValueError("CBAM bundle metadata is unavailable")
