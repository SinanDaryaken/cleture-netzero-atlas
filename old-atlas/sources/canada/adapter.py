...
from sources.canada.parser import CanadaDocument, CanadaParser


class CanadaAdapter(BaseAtlasSourceAdapter):
    VERSION = "3.0"
    ...
    async def check(self, context: PipelineContext) -> ChangeCheckResult:
        ...
        result = ChangeCheckResult(
            ...
            reason=(
                "Canada Version 3.0 regulatory data tables changed"
                if changed
                else "Canada Version 3.0 regulatory data tables unchanged"
            ),
        )
    ...
    async def fetch(self, context: PipelineContext) -> FetchedAsset:
        ...
        if self._asset is None:
            raise ValueError("check must download the Canada document before fetch")
        return self._asset
    ...
    async def parse(self, context: PipelineContext) -> tuple[ParsedRecord, ...]:
        if context.fetched_asset is None or context.fetched_asset.local_path is None:
            raise ValueError("Canada parser requires a local HTML path")
    ...
    async def normalize(self, context: PipelineContext) -> tuple[CanonicalFactor, ...]:
        if context.raw_asset is None or self._document is None:
            raise ValueError("Canada parsed document is unavailable")
    ...
    def _version_key(self, context: PipelineContext) -> str:
        if context.raw_asset is None or self._revision is None:
            raise ValueError("Canada raw metadata is unavailable")
        return f"v{self.VERSION}-{self._revision[:12]}"
